            ...
            with open(os.path.join(directory, filename), 'r') as reader:
                for line in reader:
                    if 'Execution' in line:
                        timeStr = re.findall("\-?\d+\.\d+|\-?\d+", line)
                        time = float(timeStr[0])
...
def processConnections():
    global cost
    connections = []
    directory = f'C:\\Users\\mn170387d\\Desktop\\clusters\\connections.conn'
...
def main():
    ...
    getExecTimes()
    print("Total execution time on GPU: ", round(totalTime / (1000 * 60), 2), "min")
    plt.show()
